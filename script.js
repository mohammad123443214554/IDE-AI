...
// URL ekdum clean hona chahiye, bina kisi extra space ke
...
async function generateWebsite() {
    ...
    if (!userPrompt) return alert("Bhai, pehle kuch likho!");
    ...
    chatHistory.innerHTML += `
        <div id="${loadingId}" class="flex gap-3 mb-4">
            ...
            <div class="bg-gray-800 border border-gray-700 p-3 rounded-xl rounded-tl-none text-sm text-gray-100 italic">Bhai, AI website build kar raha hai... 🚀</div>
        </div>`;
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                contents: [{ parts: [{ text: `Generate a complete responsive single-file HTML website with Tailwind CSS for: ${userPrompt}. Include CSS and JS in the same file. Return only code.` }] }]
            })
        });
...
// Run Button Logic (Naye tab mein open karne ke liye)
